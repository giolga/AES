...
int main() { 
    string message = "Two One Nine Twochama"; 
    string key = "Thats my Kung Fu"; 

    // --- Key Preparation (OK for AES-128) ---
    key = key_check(&key); 
    cout << "Key Size: " << key.size() << " _ " << key << "'" << endl; 
    
    string padded_message = message_padding(message);
    
    vector<string> chunks = split_into_chunks(&padded_message); 
    
    cout << chunks.size() << endl; // Should be 2 (21 bytes -> 32 padded bytes -> 2 chunks)
    ...
    cout << "Build state matrix: \n";

    for(int i = 0; i < chunks.size(); i++) {
        vector<vector<unsigned char>> state = build_state_matrix(chunks[i]);
        cout << "State: " << endl;
        print_matrix(state);
